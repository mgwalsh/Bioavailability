---
title: Spectrometry workflows for predicting zinc levels in maize grain
...
In this notebook, we shall explore if Zn concentrations in maize grain can be reliably predicted from cheap fast, non-destructive and non-hazardous MIR spectral data (features), using [ICP-MS](https://en.wikipedia.org/wiki/Inductively_coupled_plasma_mass_spectrometry) as reference (label) data. The reference analytical methods are described in [Gashu et al (2021)](https://doi.org/10.1038/s41586-021-03559-3). 

The two main empirical questions posed here are:

* Can Zn concentrations in soils be used to predict Zn concentrations in maize grain? While there should be strong bio-kinetic links between soils and plants, the differential uptake and bioavailability of soil Zn in food crop tissues is complex and is not well quantified or predicted. 
...
The notebook is also intended for self-learning in [R](https://www.r-project.org/). It does not go into the details of the underlying [spectroscopy](https://en.wikipedia.org/wiki/Spectroscopy) that is widely covered in the literature, but instead it focuses on [spectrometry](https://en.wikipedia.org/wiki/Spectrophotometry) and the associated computing workflows that are needed to generate useful predictions from a population of spectral signatures of (features) relative to their corresponding reference measurements (labels). In this particular example, we shall use maize grain Zn reference data, and MIR spectral data that were collected as part of the ongoing [GeoNutrition project](http://www.geonutrition.com) in Malawi. The described workflows might also be transferable to diagnosing other MNDs in soils and crops and/or to other geographies via e.g., [transfer learning](https://en.wikipedia.org/wiki/Transfer_learning) and/or other approaches ... tbd.
...
The following chunk then writes out the data frames `MW_soil_Zn_data.csv` and `MW_crop_Zn_data.csv` into your `./MW_Zn_data/Results` directory if you'd like to process those outputs in software other than R. It also generates a location map of where in Malawi those soil and plant samples were obtained.
...
# Predicting maize grain Zn from soil properties

This section introduces the main MLA workflow of the notebook. It uses topsoil (0-20 cm) reference wet chemistry features to predict maize grain Zn concentrations. The heuristics that might be used for diagnosing Zn deficiencies in crop tissues, given [gold-standard](https://en.wikipedia.org/wiki/Gold_standard_(test)) reference criteria and measurements on both soils and crops are constrained. Soils are relatively easy to collect and measure in an appropriate laboratory setting over the course of any given year; whereas, the collection and analysis of grain samples is restricted to very short time periods immediately after harvest. So, gold standard measurements will probably be quite hard to achieve in most operational and monitoring applications in Africa given the associated expenses, time constraints and lab delays. Nonetheless, they provide a baseline to compare to the spectral models that are developed in subsequent sections of the notebook. 
