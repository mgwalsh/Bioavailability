...
```{r}
# Package names
packages <- c("downloader", "devtools", "leaflet", "caret", "caretEnsemble", "plyr", "MASS", "randomForest", "gbm", "xgboost", "Cubist", "pls", "glmnet", "quantreg", "doParallel")
...
cpcas <- cpcas[ ,1:20] ## save the first 25 components, which explain ~95% of the total crop spectral variability
...
# Calibration features
ccal <- cal[ ,3595:3619] ## crop spectral PCAs
...
The initial maize grain Zn prediction using crop MIR spectra does not look as promising as those derived from the soil data, which is somewhat counterintuitive given that the MIR spectra were measured on the grain itself. So, we will try a variable regularization and selection approach here that fits 3 additional MLAs, `pls`, `glmnet` and `xgbLinear`, and will then re-ensemble all of the available individual predictions. Note that this calibration step can take up to ~1 hour to run on a normal computer with 8 cores and 16 Gb of RAM.
