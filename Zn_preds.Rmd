...
This is the `slist` prediction plot below. Note that the intermediate chunks are not shown in this html; however, they are included in the script should you wish to check.
...
We can also compare how the spectral predictions perform relative to the much more expensive and time consuming soil wet chemistry based predictions. The figure below shows that relationship.
...
**The main takeaway from this section is that maize grain Zn levels are reliably predicted using soil MIR spectra and simple, quite fast MLAs for calibration and prediction of co-located maize grain Zn levels.** In this particular case there was actually no need to use to use much slower and/or computationally more challenging algorithms such as `pls`, `glmnet` and/or `xgbLinear` that are often used for regulariztion and variable selection in high dimensional, chemometric datasets. So, we have not pursued those here. A simple PCA data reduction step of the raw MIR spectra appears to have been quite sufficient in this case.  
