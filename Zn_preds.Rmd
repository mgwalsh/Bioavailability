...
The predictions are clearly offset, but we can adjust for that. This does not alter the notion that the wet chemistry soil properties do indeed appear to be quite useful for predicting maize grain Zn levels in Malawi, plus or minus some noise. The next short chunk adjusts for the 50% quantile (median) offset that is used to generate the associated figure below.  

```{r}
# Offset adjustments
sdata$sw05 <- swQ$coefficients[2]*sdata$sw+swQ$coefficients[1] ## 5% quantile
sdata$sw50 <- swQ$coefficients[4]*sdata$sw+swQ$coefficients[3] ## 50% quantile
sdata$sw90 <- swQ$coefficients[6]*sdata$sw+swQ$coefficients[5] ## 90% quantile
```
...
Other algorithms such as [Partial least squares regression (PLS)](https://en.wikipedia.org/wiki/Partial_least_squares_regression), [Ridge regression](https://en.wikipedia.org/wiki/Ridge_regression) and/or [Lasso regression](https://en.wikipedia.org/wiki/Lasso_(statistics)) handle the high dimensional data and collinearities well in a linear context but are frequently not very good at predicting common non-linear and/or threshold relationships between the spectral signatures and their reference measurements.
