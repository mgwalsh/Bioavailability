...
The predictions are clearly offset, but we can adjust for that. Note that this does not distract from the notion that the wet chemistry soil properties do indeed appear to be quite useful for predicting maize grain Zn levels in Malawi, plus or minus some outlier noise. The next short chunk adjusts for the 50% quantile (median) offset and is used to generate the associated figure below.  
...
sspec.pca <- prcomp(sdata[ ,18:3594], center=T, scale=T) ## centered and scaled PCAs
...
## Calibration with soil spectra and `caretEnsemble`
