...
```{r, fig.align = "center", fig.cap = "Relationship between predicted (based on soil wet chemistry properties) and measured maize grain zinc (Zn) levels."}
...
While these predictions are clearly offset, we can adjust for that by debiasing. That does not alter the notion that soil properties via various lab tests, do indeed appear to be quite predictive for maize grain Zn levels in Malawi. See the short example chunk for for debiasing and figure below.  
...
```{r, echo = FALSE, fig.align = "center", fig.cap = "Relationship between debiased predictions (based on soil wet chemistry properties) and measured maize grain zinc (Zn) levels."}
