...
The initial maize grain Zn prediction using crop MIR spectra does not look as promising as those derived from the soil data, which is somewhat odd given that the MIR spectra were measured on the grain itself. So we will try a variable regularization and selection approach here that fits 3 additional MLAs, `pls`, `glmnet` and `xgbLinear`, and will then re-ensemble the predictions. Note that the calibrations can take up to ~1 hour to run on a normal computer with 8 cores and 16 Gb of RAM.
