...
While these predictions are clearly offset we can adjust for the offset by debiasing them using the following chunck. This would adjust the previous graph, but does not alter the notion that indeed soil properties are indeed linked to Zn concentrations in maize grain in Malawi.  

```{r}
# Debiasing
sdata$sw05 <- swQ$coefficients[2]*sdata$sw+swQ$coefficients[1]
sdata$sw50 <- swQ$coefficients[4]*sdata$sw+swQ$coefficients[3]
sdata$sw90 <- swQ$coefficients[6]*sdata$sw+swQ$coefficients[5]
```
