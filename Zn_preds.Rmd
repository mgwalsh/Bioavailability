...
We initially calibrate 3 MLAs to the soil spectral principal components data with 10-fold cross-validation. Note that the calibration code pattern is similar to the wet-chemistry-based pattern that was generated above. We just substitute the soil spectral principal components for the wet chemistry features but are running the same MLAs for comparison.
...
This is the `slist` prediction plot below. Note that the intermediate chunks are not shown in this html; however, they are in the script should you wish to check.
...
# Quantile regression fit on whole dataset
swQ <- rq(Zn~ss_slist, tau=c(0.05,0.5,0.95), data = sdata)
summary(swQ)
```

```{r, echo = FALSE}
# Offset adjustment
sdata$ss50 <- swQ$coefficients[4]*sdata$sw+swQ$coefficients[3] ## 50% quantile adjustment

# Refit quantile regression
ss50Q <- rq(Zn~ss50, tau=c(0.05,0.5,0.95), data = sdata)
```

```{r, echo = FALSE, fig.align = "center", fig.cap = "Relationship between median adjusted predictions from soil spectral data and measured maize grain zinc (Zn) levels."}

par(pty="s", mar=c(4,4,1,1))
plot(Zn~ss50, xlab="Offset adjusted ensemble grain Zn prediction (ppm)", ylab="Measured grain Zn (ppm)", cex.lab=1.2,
     xlim=c(10,50), ylim=c(10,50), sdata)
