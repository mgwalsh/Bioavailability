...
**The main takeaway from this section is that maize grain Zn levels are reliably predicted using soil MIR spectra and simple, quite fast MLAs for calibration and prediction of co-located maize grain Zn levels.** In this particular case there was actually no need to use to use much slower and/or computationally challenging algorithms such as `pls`, `glmnet` and/or `xgbLinear` that are often used for regulariztion and variable selection in high dimensional, chemometric datasets. So, we have not pursued here. A simple PCA data reduction step of the raw spectra appears to have been quite sufficient in this case.  
