...
* There is potential scope within the [GeoNutrition project](http://www.geonutrition.com) to also test this approach for other mineral micronutrients (e.g., iron, iodine and selenium) that might of concern in particular regions of interest and/or about other crops.

* Computationally, all of the code presented in this notebook runs fast and probably could be automated, such that when a (soil) MIR measurement is received in a laboratory a maize grain Zn prediction could be issued in near-real time. Note that other spectrometer setups and the associated calibration transfer steps might need to be taken into account in that regard.   
