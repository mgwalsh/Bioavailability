...
plot(uZn~pDM, wetdat, xlab="Plant dry matter (mg / pot)", xlim=c(0,2500), ylab="Zn uptake (mg / pot)", ylim=c(0,25), cex=1.1, cex.lab=1.5)
...
# print(ZnQ)
# summary(ZnQ)
# plot(pDM~uZn, wetdat, xlab="Zn uptake (mg / pot)", xlim=c(0,25), ylab="Dry matter (mg / pot)", ylim=c(0,2500), cex=1.1, cex.lab=1.5)
...
print(ZnS)
summary(ZnS)
plot(uZn~Zn, wetdat, xlab="Soil Zn (ppm)", xlim=c(0,200), ylim=c(0,25), ylab="Zn uptake (mg / pot)", cex=1.1, cex.lab=1.5)
...
# Zn uptake index as f(Zn concentration in soil & plant DM yield)
...
plot(uZn~Zn50, wetdat, xlab="Zn uptake index (mg / pot)", xlim=c(0,25), ylim=c(0,25), ylab="Zn uptake (mg / pot)", cex=1.1, cex.lab=1.5)
abline(c(0,1), col="grey", lwd=2)

# B uptake as f(DM yield)
wetdat$uB <- wetdat$pDM/10000*wetdat$pB ## B uptake/pot
BY <- rq(log(uB)~log(pDM), tau=c(0.025,0.5,0.975), data=wetdat)
print(BY)
summary(BY)
par(mfrow=c(1,3), mar=c(5,5,1,1))
plot(uB~pDM, wetdat, xlab="Plant dry matter (mg / pot)", xlim=c(0,2500), ylab="B uptake (mg / pot)", ylim=c(0,10), cex=1.1, cex.lab=1.5)
curve(exp(BY$coefficients[1])*x^BY$coefficients[2], add=T, from=0, to=2500, col="blue", lwd=2)
curve(exp(BY$coefficients[5])*x^BY$coefficients[6], add=T, from=0, to=2500, col="blue", lwd=2)
curve(exp(BY$coefficients[3])*x^BY$coefficients[4], add=T, from=0, to=2500, col="red", lwd=2)

# B uptake as f(B concentration in soil)
BS <- rq(log(uB)~log(B), tau=c(0.025,0.5,0.975), data=wetdat) ## B uptake/pot
print(BS)
summary(BS)
plot(uB~B, wetdat, xlab="Soil B (ppm)", xlim=c(0,10), ylim=c(0,10), ylab="B uptake (mg / pot)", cex=1.1, cex.lab=1.5)
curve(exp(BS$coefficients[1])*x^BS$coefficients[2], add=T, from=0, to=10, col="blue", lwd=2)
curve(exp(BS$coefficients[5])*x^BS$coefficients[6], add=T, from=0, to=10, col="blue", lwd=2)
curve(exp(BS$coefficients[3])*x^BS$coefficients[4], add=T, from=0, to=10, col="red", lwd=2)

# B uptake index as f(B concentration in soil & plant DM yield)
BI <- rq(log(uB)~log(B)*log(pDM), tau=c(0.025,0.5,0.975), data=wetdat) ## Zn uptake/pot
print(BI)
summary(BI)
BI.fit <- as.data.frame(exp(predict(BI, wetdat)))
names(BI.fit) <- c("B025","B50","B975")
wetdat <- cbind(wetdat, BI.fit)
plot(uB~B50, wetdat, xlab="B uptake index (mg / pot)", xlim=c(0,2), ylim=c(0,10), ylab="B uptake (mg / pot)", cex=1.1, cex.lab=1.5)
abline(c(0,1), col="grey", lwd=2)
